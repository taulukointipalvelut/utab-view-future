        ...
        el-form
          el-form-item(label="PM", required, error="Select PM's Name")
            el-select(:value="og.result.leader.id", @input="on_og_pos_name('leader', $event)", placeholder="Select PM")
              el-option(v-for="id in team_by_id(score_sheet.teams.og).speakers", :key="id", :label="speaker_by_id(id).name", :value="id")
        ...
        el-form
          el-form-item(label="LO", required, error="Select LO's Name")
            el-select(:value="oo.result.leader.id", @input="on_oo_pos_name('leader', $event)", placeholder="Select LO")
              el-option(v-for="id in team_by_id(score_sheet.teams.oo).speakers", :key="id", :label="speaker_by_id(id).name", :value="id")
...
export default {
  ...
  methods: {
    ...
    on_og_pos_name (pos_name, value) {
      this.$store.commit('ballot/og_pos_name', { pos_name, value })
    },
    on_oo_pos_name (pos_name, value) {
      this.$store.commit('ballot/oo_pos_name', { pos_name, value })
    }
  }
}
</script>
